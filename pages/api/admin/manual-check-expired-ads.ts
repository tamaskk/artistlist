...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    if (req.method === "POST") {
        try {
            ...
            console.log(`Manual check completed: ${results.processed} processed, ${results.updated} updated, ${results.errors} errors`);
            
            res.status(200).json({
                ok: true,
                message: "Manual expired ads check completed",
                timestamp: now.toISOString(),
                results: results
            });
            
        } catch (error) {
            console.error("Error in manual expired ads check:", error);
            res.status(500).json({
                ok: false,
